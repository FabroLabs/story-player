...
export function createReactStoryPlayer(React) {
  ...
  function StoryPlayer({ story, assetBase, debug = false, children: _children, ref: _ref, ...host }) {
    const hostRef = useRef(null);
    useEffect(() => {
      const player = createStoryPlayer(hostRef.current, { story, assetBase, debug });
      void player.ready.catch(() => {
        // The plain player owns and renders its initialization error surface.
      });
      return () => player.destroy();
    }, [story, assetBase, debug]);
